...
FSR将当前深度反向重投影到上一帧的多个双线性像素来判断遮挡, 这可能导致错误的遮挡关系, 且双线性遮挡较为保守, 引发抖动.
...
[W17](https://bartwronski.com/2017/04/13/cull-that-cone/)指出使用网格包围球和锥体求交可以实现更精确的聚光灯剔除.

![cone-culling](cone-culling.svg)

令源立体角为$\Delta\Omega$, 方向光强度被定义为垂直辐照度$E=L_i\int_{\Delta\Omega}\cos\theta \mathrm{d}\omega$, 由此可得$L_i$:

$$
\begin{equation}
\begin{aligned}
L_i
&= \frac{E}{\int_{\Delta\Omega} \cos\theta \mathrm{d}\omega}\\
&= \frac{E}{\int_0^{2\pi}\int_0^{\theta_0} \cos\theta\sin\theta \mathrm{d}\theta\mathrm{d}\phi}\\
&= \frac{E}{\frac{\pi}{2}(1 - \cos2\theta_0)}
\end{aligned}
\end{equation}
$$
...
全局距离场是以相机为中心的Clipmap, 每级范围与体素粒度翻倍, 运行时用环形寻址加载, 相机移动只需重建新暴露出的一层. 每$8^3$个体素占一页, 每个体素8位. 页表是每级一张三维纹理, 记录该页在页图集中的索引, 只有存在表面的页才从图集分配. 页只负责$8^3$个体素中心构成的包围盒, 即能三线性插值的部分; 超出包围盒的左右各半个体素由其它页负责, 这也导致相邻页在每个轴上重叠一层. 每页存储的距离场根据页宽度归一化.

合成先按包围盒确定每个物体影响的页, 更新时一个线程组负责一页, 每个体素读取候选物体的距离场并取有符号距离的最小值, 也就是求距离场的并集. 读取前先把体素位置变换到物体局部空间, 缩放取三轴最小值以维持保守. 网格距离场只在物体包围盒内有数据, 落在盒外的体素先钳制到盒面采样, 再补上体素到盒面的距离作为近似. 静态物体合成后长期缓存, 动态物体单独一层再与之取最小值, 每帧只重建被标脏的页.

## Volume

体素锥追踪求积分而非求交, 每步的Mip由锥在该处的半径决定, 一次采样的足迹覆盖锥的横截面. 采样返回预滤波的辐照度与不透明度, 累积不透明度饱和或走出体积即终止.

$$
\begin{equation}
C \mathrel{+}= (1-\alpha)\alpha_i c_i
\qquad
\alpha \mathrel{+}= (1-\alpha)\alpha_i
\end{equation}
$$
