...
$$
\begin{equation}
E(p) = \frac{\mathrm{d}\Phi}{\mathrm{d}A}
\end{equation}
$$
...
$$
\begin{equation}
E(p) = \frac{\mathrm{d}\Phi \cos\theta}{\mathrm{d}A}
\end{equation}
$$
...
\begin{equation}
L(p, \omega)
= \frac{E_{\omega}(p)}{\mathrm{d}\omega}
= \frac{d^2\Phi}{\mathrm{d}\omega \mathrm{d}A^{\perp}}
= \frac{d^2\Phi}{\mathrm{d}\omega \mathrm{d}A \cos\theta}
\end{equation}
$$

由于随着传播范围增大, 单位面积的辐射通量减小, 但同时它所占的立体角也减小,
...
L_i(p, \omega) =
\begin{cases}
L^+(p, -\omega), & \omega \cdot \mathbf{n}_p > 0\\
L^-(p, -\omega), & \omega \cdot \mathbf{n}_p < 0
\end{cases}
\end{equation}
$$
...
$$
\begin{equation}
Y = \int_{\lambda} L_\lambda(\lambda) V(\lambda) \mathrm{d}\lambda
\end{equation}
$$
...
$$
\begin{equation}
E(p, \mathbf{n}) = \int_\Omega L_i(p,\omega) |\cos\theta| \mathrm{d}\omega
\end{equation}
$$
...
BRDF代表双向反射分布函数, 我们需要入射辐亮度对某方向出射辐亮度的影响, 由于入射光照的辐亮度变化对出射辐亮度的影响是线性的, 因此可以用除法定义:
...
$$
\begin{equation}
\rho_{hh} = \int_\Omega \frac{\int_\Omega f_r(p, \omega_o, \omega_i) \cos\theta_i \mathrm{d}\omega_i}{\int_\Omega \cos\theta_i \mathrm{d}\omega_i} \cos\theta_o \mathrm{d}\omega_o
\end{equation}
$$
...
$$
\begin{equation}
L_o(p, \omega_o) = \int_\Theta f(p, \omega_o, \omega_i) L_i(p, \omega_i) |\cos\theta_i| \mathrm{d}\omega_i
\end{equation}
$$
...
$$
\begin{equation}
S(p_o, \omega_o, p_i, \omega_i) = \frac{\mathrm{d}L_o(p_o, \omega_o)}{\mathrm{d}\Phi(p_i, \omega_i)}
\end{equation}
$$
...
$$
\begin{equation}
L_o(p_o, \omega_o) = \int_A \int_\Omega S(p_o, \omega_o, p_i, \omega_i) L_i(p_i, \omega_i) |\cos\theta_i| \mathrm{d}\omega_i \mathrm{d}A
\end{equation}
$$
...
$$
\begin{equation}
\frac{\int_\lambda \Phi_e(\lambda) V(\lambda) \mathrm{d}\lambda}{\int_\lambda \Phi_i(\lambda) \mathrm{d}\lambda}
\end{equation}
$$
...
$$
\begin{equation}
v_i = \int_\lambda S(\lambda)m_i(\lambda)\mathrm{d}\lambda
\end{equation}
$$
...
\begin{equation}
\begin{aligned}
x_\lambda &= \frac{\int_\lambda S(\lambda)X(\lambda)\mathrm{d}\lambda}{\int_\lambda Y(\lambda)\mathrm{d}\lambda}\\
y_\lambda &= \frac{\int_\lambda S(\lambda)Y(\lambda)\mathrm{d}\lambda}{\int_\lambda Y(\lambda)\mathrm{d}\lambda}\\
z_\lambda &= \frac{\int_\lambda S(\lambda)Z(\lambda)\mathrm{d}\lambda}{\int_\lambda Y(\lambda)\mathrm{d}\lambda}
\end{aligned}
\end{equation}
$$
...
\end{bmatrix} =
\begin{pmatrix}
\int_\lambda R(\lambda) X(\lambda)\mathrm{d}\lambda & \int_\lambda G(\lambda) X(\lambda)\mathrm{d}\lambda & \int_\lambda B(\lambda) X(\lambda)\mathrm{d}\lambda\\
\int_\lambda R(\lambda) Y(\lambda)\mathrm{d}\lambda & \int_\lambda G(\lambda) Y(\lambda)\mathrm{d}\lambda & \int_\lambda B(\lambda) Y(\lambda)\mathrm{d}\lambda\\
\int_\lambda R(\lambda) Z(\lambda)\mathrm{d}\lambda & \int_\lambda G(\lambda) Z(\lambda)\mathrm{d}\lambda & \int_\lambda B(\lambda) Z(\lambda)\mathrm{d}\lambda
\end{pmatrix}
\begin{bmatrix}
r\\
g\\
...
`RGBColorSpace`的构造需要提供三原色的色度以及白点光谱分布. 通过三原色可以确定从XYZ到当前色彩空间的变换矩阵. 白点色度转换首先将白点从XYZ转到当前色彩空间, 然后应用缩放矩阵, 这使得当前色彩空间下的白色$(1.0, 1.0, 1.0)$可以映射到正确的颜色上.

白点规定亮度, 因此色彩空间只需定义色度, 不同亮度产生不同缩放矩阵, 为满足白点要求, 任意亮度的缩放矩阵和白点缩放矩阵的乘法结果相同. 矩阵表达式与构造函数代码如下.
...
XYZ如何对颜色归一化是不确定的, 因此通常作为相对颜色来使用. 由于HDR标准要求提供绝对亮度, 需要确定$Y=1$对应的亮度, pbrt下即为CIE Y的积分.
