...
\begin{aligned}
A
&= \phi_{\max} \int_a^b f(x) \mathrm{d}s\\
&= \phi_{\max} \int_a^b f(x) \sqrt{\mathrm{d}x^2 + \mathrm{d}y^2}\\
&= \phi_{\max} \int_a^b f(x) \sqrt{1 + f'^2(x)} \mathrm{d}x
\end{aligned}
\end{equation}
$$
...
\begin{aligned}
A
&= \phi_{\max} \int_{z_{\min}}^{z_{\max}} \sqrt{r^2 - z^2} \sqrt{1 + \frac{z^2}{r^2 - z^2}} \mathrm{d}z\\
...
\begin{aligned}
1
&= \int_{\Phi} p(\omega) \mathrm{d}\omega\\
&= \int_0^{2\pi} \int_0^{\theta_{\max}} p(\phi)p(\theta) \sin\theta \mathrm{d}\theta \mathrm{d}\omega
\end{aligned}
\end{equation}
$$
...
\begin{aligned}
1
&= c \int_0^{\theta_{\max}} \sin\theta \mathrm{d}\theta\\
&= c (1 - \cos\theta_{\max})
\end{aligned}
\end{equation}
$$
...
S =
\begin{pmatrix}
1 & 0 & -\frac{\mathbf{d}_x}{\mathbf{d}_z} & 0\\
0 & 1 & -\frac{\mathbf{d}_y}{\mathbf{d}_z} & 0\\
0 & 0 & \frac{1}{\mathbf{d}_z} & 0\\
0 & 0 & 0 & 1
\end{pmatrix}
\end{aligned}
\end{equation}
...
\mathbf{c'}&=\cos\overset{\frown}{ac'}\mathbf{a}+\sin\overset{\frown}{ac'}\mathbf{c_\perp}
\end{aligned}
\end{equation}
$$

将$b$变换到球的北极, 根据微分立体角转微分面积的公式, 以及经过$b$的两平面的夹角$\beta$与所占方位角$\phi$相等这一关系, $c''$处与$\overset{\frown}{bc'}$垂直的微分圆弧长度为$\sin\overset{\frown}{bc'}d\overset{\frown}{bc'}\mathrm{d}\beta$. 此时可根据$\overset{\frown}{bc'}$上的积分求出微分三角形的面积.

$$
\begin{equation}
\begin{aligned}
A_{\overset{\frown}{bc''}}=\int_0^{\overset{\frown}{bc''}} \sin\theta \mathrm{d}\theta \mathrm{d}\beta=(1-\cos\overset{\frown}{bc''})\mathrm{d}\beta
\end{aligned}
\end{equation}
$$

根据$A_{\overset{\frown}{bc''}}$与$A_{\overset{\frown}{bc'}}$的比值, 可得根据$\epsilon_1$采样$c''$的方式, 向量$\mathbf{c''}$即为采样方向$\omega$. 由于不再需要判断光线是否相交, 此时根据$o+t\mathbf{d}=(1-b_1-b_2)v_0+b_1v_1+b_2v_2$求解重心坐标即可.
...
对于需要根据采样方向倒推PDF的情况, 由于$c''$已知, 根据$\overset{\frown}{bc''}$与$\overset{\frown}{ac}$的交点可求得$c'$. 交点为圆弧构成的平面的相交直线与球的交点, 直线方向通过两平面法线叉乘求得, 选择与$\mathbf{a}+\mathbf{c}$同向的交点, 同时已知交线会经过球心, 此时可以确定$c'$以及$abc'$的面积, 以此求得$\epsilon_0$. $\epsilon_1$根据概率公式直接求出即可.
...
\begin{equation}
\begin{aligned}
a &= ((p_{1,0} - p_{0,0}) \times (p_{0,1} - p_{1, 1})) \cdot \mathbf{d}\\
c &= (p_{0,0} - \mathbf{o}) \times \mathbf{d}) \cdot (p_{0,1} - p_{0,0})\\
b &= (p_{1,0} - \mathbf{o}) \times \mathbf{d}) \cdot (p_{1,1} - p_{1,0}) - (a + c)
\end{aligned}
\end{equation}
$$
...
\begin{equation}
\begin{aligned}
t &= \frac{\text{det}(f_u(0) - \mathbf{o}, f_u(1) - f_u(0), \mathbf{d} \times (f_u(1) - f_u(0)))}{\Vert \mathbf{d} \times (f_u(1) - f_u(0)) \Vert^2}\\
v &= \frac{\text{det}(f_u(0) - \mathbf{o}, \mathbf{d}, \mathbf{d} \times (f_u(1) - f_u(0)))}{\Vert \mathbf{d} \times (f_u(1) - f_u(0)) \Vert^2}
\end{aligned}
\end{equation}
$$
...
\begin{equation}
\begin{aligned}
t = (x \ominus o_x) &\otimes (1 \oslash \mathbf{d}_x) \in \frac{x - o_x}{\mathbf{d}_x}(1 \pm \epsilon)^3\\
\frac{x - o_x}{\mathbf{d}_x} &\in t (1 \pm \gamma_3)\\
\delta_r &= \gamma_3 |t|
\end{aligned}
\end{equation}
$$
...
&= 4a\left((\mathbf{o} \cdot \hat{\mathbf{d}})^2 - (\mathbf{o} \cdot \mathbf{o}) + r^2\right)\\
&= 4a\left(-\Vert \mathbf{o} - \frac{b}{2a}\mathbf{d} \Vert^2 + r^2\right)
\end{aligned}
\end{equation}
$$
...
$$
\begin{equation}
b^2 - 4ac = 4a \left(r^2 - \Vert \mathbf{o_{xy}} - \frac{b}{2a} \mathbf{d_{xy}} \Vert^2\right)
\end{equation}
$$
...
$$
\begin{equation}
\delta_a = \gamma_1|o_x|+\delta_t(1 + \gamma_2)|\mathbf{d_x}|+\gamma_2|t_{\text{hit}}\mathbf{d_x}|
\end{equation}
$$
...
$$
\begin{equation}
\mathbf{n} \cdot (\delta_x, \delta_y, \delta_z)
\end{equation}
$$
...
\begin{equation}
\begin{aligned}
c_{\max}(\mathbf{v}) &= \max(\mathbf{v_x}, \mathbf{v_y}, \mathbf{v_z})\\
\epsilon
&= \gamma_10 (c_{\max}(|\mathbf{o}|) + c_{\max}(|\mathbf{d}|)\\
&+ c_{\max}(|\mathbf{p_{0,0}}|)+ c_{\max}(|\mathbf{p_{0,1}}|)\\
&+ c_{\max}(|\mathbf{p_{1,0}}|)+ c_{\max}(|\mathbf{p_{1,1}}|))
\end{aligned}
\end{equation}
$$
