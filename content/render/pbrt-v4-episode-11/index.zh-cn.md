...
$$
\begin{equation}
\mathrm{d}L_o(p,\omega)=\sigma_a(p,\omega)L_e(p,\omega)\mathrm{d}t
\end{equation}
$$
...
$$
\begin{equation}
\mathrm{d}L_o(p,\omega)=\sigma_s(p,\omega)\int_\Theta p(p,\omega_i,\omega)L_i(p,\omega_i)\mathrm{d}\omega_idt
\end{equation}
$$
...
\begin{equation}
\begin{aligned}
\mathrm{d}L_o(p,\omega)
&=\sigma_t(p,\omega)L_s(p,\omega)\mathrm{d}t\\
&=\sigma_t(p,\omega)(\frac{\sigma_a(p,\omega)}{\sigma_t(p,\omega)}L_e(p,\omega)+\frac{\sigma_s(p,\omega)}{\sigma_t(p,\omega)}\int_\Theta p(p,\omega_i,\omega)L_i(p,\omega_i)\mathrm{d}\omega_i)\mathrm{d}t
\end{aligned}
\end{equation}
$$
...
$$
\begin{equation}
\tau(p \to p')=\int_0^d \sigma_t(p+t\omega,\omega)\mathrm{d}t
\end{equation}
$$

路径追踪任务是光线传播的逆过程, 因此$-\mathrm{d}L(p)=-\sigma_t(p,\omega)L(p)\mathrm{d}t$. pbrt中下列公式都是由这一条件推导出的, 但是并没有注明, 因此尝试推导时会得到相反的结论, 具体推导过程见[Integral formulations of volumetric transmittance](https://dl.acm.org/doi/pdf/10.1145/3355089.3356559).

根据微分方程可以得到$L(p,\omega)-L(p+\mathrm{d}\omega,\omega)=\int_0^d -\sigma_t(p+t\omega)L(p+t\omega,\omega)\mathrm{d}t$, 已知$T_r(p \leftarrow p + \mathrm{d}\omega)=\frac{L(p,\omega)}{L(p + \mathrm{d}\omega,\omega)}$, 代入前式可以得到以下积分. 此时可以通过Monte Carlo得到透射率的无偏估计, 但较为复杂.

$$
\begin{equation}
T_r(p \leftarrow p + \mathrm{d}\omega)=1 - \int_0^d \sigma_t(p+t\omega)T_r(p + t\omega \leftarrow p+\mathrm{d}\omega)\mathrm{d}t
\end{equation}
$$
...
$$
\begin{equation}
-\frac{\mathrm{d}L(p,\omega)}{\mathrm{d}t}+\sigma_{\text{maj}}(p,\omega)L(p,\omega)=\sigma_n(p,\omega)L(p,\omega)
\end{equation}
$$
...
\begin{equation}
\begin{aligned}
h(t)&=e^{-\int_0^t \sigma_{\text{maj}}(p+t'\omega,\omega) \mathrm{d}t'}\\
\frac{dh(t)}{\mathrm{d}t}&=-h(t)\sigma_{\text{maj}}(p+t\omega,\omega)\\
...
\begin{equation}
\begin{aligned}
-h(t)\frac{\mathrm{d}L(p+t\omega,\omega)}{\mathrm{d}t}-L(p+t\omega,\omega)\frac{dh(t)}{\mathrm{d}t}&=h(t)g(t)\\
h(0)L(p,\omega)-h(d)L(p+\mathrm{d}\omega,\omega)&=\int_0^d h(t)g(t)\mathrm{d}t
\end{aligned}
\end{equation}
$$

两边同除$h(0)L(p+\mathrm{d}\omega,\omega)$后可得下式.

$$
\begin{equation}
\begin{aligned}
T_r(p \leftarrow p+\mathrm{d}\omega)
&=\frac{h(d)}{h(0)}+\int_0^d \frac{h(t)g(t)}{h(0){L(p+\mathrm{d}\omega,\omega)}}\\
&=e^{-\int_0^d \sigma_\text{maj}(p+t\omega,\omega)\mathrm{d}t}+\int_0^d e^{-\int_0^t \sigma_\text{maj}(p+t'\omega,\omega)\mathrm{d}t'}\sigma_n(p+t\omega,\omega)T_r(p+t\omega \leftarrow p+\mathrm{d}\omega)\mathrm{d}t
\end{aligned}
\end{equation}
$$

计算上式的Monte Carlo需要使用一个与积分项成比例的分布, pbrt使用$p_{\text{maj}}(t) \propto e^{\int_0^t -\sigma_{\text{maj}}(p+t'\omega, \omega)\mathrm{d}t'}$, 归一化后得到以下PDF.

$$
\begin{equation}
p_{\text{maj}}(t)=\sigma_{\text{maj}}(p,\omega)e^{-\int_0^t \sigma_{\text{maj}}(p+t'\omega,\omega) \mathrm{d}t'}
\end{equation}
$$
...
$$
\begin{equation}
T_r(p \leftarrow p+\mathrm{d}\omega) \approx e^{-\int_0^\mathrm{d}\sigma_{\text{maj}}(p+t\omega,\omega)\mathrm{d}t}+
\begin{cases}
\frac{\sigma_n(p+t'\omega,\omega)}{\sigma_{\text{maj}}(p+t'\omega,\omega)}T_r(p+t'\omega \leftarrow p+\mathrm{d}\omega) & t'<d\\
0 & \text{otherwise}
\end{cases}
\end{equation}
$$

随机选取其中一项也可以进行Monte Carlo, 由于CDF具有$P(t' \ge d)=e^{-\int_0^\mathrm{d}\sigma_{\text{maj}}\mathrm{d}t}$的性质, 可以将该值作为选择概率. 注意到下一跳估计器中是在$(0,\infty)$上的积分, 而这里是在$(0,d)$上的积分, 因此需要通过将PDF除以$1-e^{-\int_0^\mathrm{d}\sigma_{\text{maj}}(p+t\omega,\omega)\mathrm{d}t}$来归一化, Monte Carlo时正好与$\frac{1}{1-p_e}$抵消. 此时可以得到比率跟踪(ratio-tracking)估计器, 这是pbrt采用的算法.
...
&\approx
\begin{cases}
\frac{e^{-\int_0^\mathrm{d}\sigma_{\text{maj}}(p+t\omega,\omega)\mathrm{d}t}}{p_e} & \text{with}\ \text{probability}\ p_e\\
\frac{1}{1-p_e}\int_0^d e^{-\int_0^t\sigma_{\text{maj}}(p+t'\omega,\omega)\mathrm{d}t'}\sigma_n(p+t\omega,\omega)T_r(p+t\omega \leftarrow p+\mathrm{d}\omega)\mathrm{d}t & \text{otherwise}
\end{cases}\\
&\approx
\begin{cases}
1 & t' > d\\
\frac{\sigma_n(p+t'\omega,\omega)}{\sigma_{\text{maj}}(p+t'\omega,\omega)}T_r(p+t'\omega \leftarrow p+\mathrm{d}\omega) & \text{otherwise}
\end{cases}\\
&\approx
...
\begin{equation}
\begin{aligned}
T_r(p \leftarrow p+\mathrm{d}\omega)&\approx
\begin{cases}
1 & t' > d\\
T_r(p+t'\omega \leftarrow p+\mathrm{d}\omega) & t' \le d\ \text{and}\ \text{with}\ \text{probability}\ \frac{\sigma_n(p+t'\omega,\omega)}{\sigma_{\text{maj}}}\\
0 & \text{otherwise}
\end{cases}
\end{aligned}
\end{equation}
...
只与入射方向和出射方向的夹角有关的相位函数被称为对称相位函数, 否则就是需要用四个维度表示的非对称相位函数. 各个方向均匀分布的相位函数为各向同性相位函数, 只有$p(\omega_i,\omega_o)=\frac{1}{\int_0^{2\pi}\int_0^\pi \sin\theta \mathrm{d}\theta \mathrm{d}\phi}=\frac{1}{4\pi}$满足该条件, 否则为各向异性.
...
$$
\begin{equation}
g=\int_\Theta p(-\omega\cdot\omega')(\omega\cdot\omega')\mathrm{d}\omega'=2\pi\int_0^\pi p(-\cos\theta)\cos\theta\sin\theta \mathrm{d}\theta
\end{equation}
$$
