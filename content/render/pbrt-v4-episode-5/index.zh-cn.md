...
基类`CameraBase`在`GenerateRayDifferential`实现了光线微分的计算, 通过多次调用`GenerateRay`来实现. 该函数计算dx与\mathrm{d}y, 为防止超出图像范围, 正负两侧都会尝试计算. 由于pbrt不使用虚函数, 派生类需要实现相关函数并把`this`传入.
...
$$
\begin{equation} \frac{1}{z'} = \frac{1}{z} + \frac{1}{f}
\end{equation}
$$
...
\begin{aligned}
E(p) 
&= \int_{A_e} L_i(p, p') \frac{|\cos\theta \cos\theta'|}{\Vert p' - p \Vert^2} \mathrm{d}A_e\\
&= \frac{1}{z^2_f} \int_{A_e} L_i(p, p') |\cos\theta|^4 \mathrm{d}A_e
\end{aligned}
\end{equation}
$$
...
$$
\begin{equation}
H(p) = \int_{t_0}^{t_1} \frac{1}{z^2_f} \int_{A_e} L_i(p, p', t') |\cos\theta|^4 \mathrm{d}A_e \mathrm{d}t'
\end{equation}
$$
...
$$
\begin{equation}
J = \int_{A_p} \int_{t_0}^{t_1} \frac{1}{z^2_f} \int_{A_e} L_i(p, p', t') |\cos\theta|^4 \mathrm{d}A_e \mathrm{d}t' \mathrm{d}A_p
\end{equation}
$$
...
\begin{equation}
\begin{aligned}
p_v(\lambda) &= (\int_{\lambda_{\min}}^{\lambda_{\max}} f(\lambda) \mathrm{d}\lambda)^{-1} f(\lambda)\\
...
$$
\begin{equation}
r_f(x,y) = \int f(x - x', y - y') r(x', y') \mathrm{d}x' \mathrm{d}y'
\end{equation}
$$
...
$$
\begin{equation}
r_f(x,y) \approx (\int |f(x',y')| \mathrm{d}x' \mathrm{d}y')(\frac{1}{n} \sum_i^n \text{sign}(f(x - x_i, y - y_i)) r(x_i, y_i))
\end{equation}
$$
